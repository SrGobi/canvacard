...
function parseImg(imgString) {
  if (!imgString || typeof imgString !== 'string') {
    throw new APIError(`Invalid custom badge ('${imgString || undefined}') must be an image file 'png | jpg | jpeg | gif'`);
  }
  ...
  if (!imgCheck)
    throw new APIError(`Invalid customBackground ('${imgString || undefined}') must be an image file 'png | jpg | jpeg | gif'`);
...
function parseHex(hexString) {
  ...
  if (!hexRegex.test(hexString))
    throw new APIError(`Invalid Hex Code ('${hexString || undefined}')`);
...
function isString(param, type) {
  if (typeof param !== 'string')
    throw new APIError(`Invalid ${type} (${param || undefined}), must be a string`);
...
function isNumber(param, type) {
  if (isNaN(param))
    throw new APIError(`Invalid ${type} (${param || undefined}), must be a number`);
